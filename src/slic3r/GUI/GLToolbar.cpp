...
void GLToolbar::render_arrow(const GLCanvas3D& parent, GLToolbarItem* highlighted_item) const
{
    ...
    for (const GLToolbarItem* item : m_items)
    {
        ...
        else
        {
            if (item->get_name() == highlighted_item->get_name())
            {
                item->render_arrow(tex_id, left, left + scaled_icons_size, top - scaled_icons_size, top, (unsigned int)tex_width, (unsigned int)tex_height, (unsigned int)(m_layout.icons_size * m_layout.scale));
            }           
            left += icon_stride;
        }
    }
